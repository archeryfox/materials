...
class Container:
    ...
    def Import(cls,i=0):
        ...
        with open("tbl.csv", 'r') as fist:
            ...
            listProds = list(redr)
            if i==1:
                listProds = sorted(listProds, key=lambda x: x[0])
                print('Сортировано по названию')
                print(
                    tabulate(listProds,
                             tablefmt='grid', headers=["Название", "Цена", "Вес", "Количество"])
                )
            if i == 2:
                listProds = sorted(listProds, reverse=True, key=lambda x: x[-2])
    ...
    def Counter(self):
         ...
         memVol = self.volume
         products = Container.Import(Container)
         for dlist in products:
             ...
             # print()
             i = 0
             with open('container.csv', "a") as file:
                file.write(dlist[0]+','+str(memVol - prod2.weigh * (i-1))+","+str(self.price) +"\n")
             file.close()
             self.volume= memVol
             self.price = 0
         ...
         with open('container.csv', "r") as file:
             ...
             print(
                 tabulate(list1,
                          tablefmt='grid', headers=["Название", "Вес", "Цена"])
             )
         file.close()
